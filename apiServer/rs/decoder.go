...
func (d *decoder) getData() error {
	...
	log.Println("len(d.readers) ", len(d.readers))
	log.Println("d.readers ", d.readers)
	log.Println("d.readers[i] ", d.readers[0])
	for i := range shards {
		if d.readers[i] == nil {
			log.Println("if d.readers[i] == nil 1111111")
			repairIds = append(repairIds, i)

		} else {
			log.Println("} else {2222222")
	...
	log.Println("getData len(shards)111", len(shards))
	log.Println("getData shards111", shards)
	log.Println("getData repairIds111", repairIds)
